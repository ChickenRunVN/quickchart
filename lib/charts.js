...
// Polyfills
// canvas-5-polyfill removed: canvas 3.x provides Path2D and ellipse natively.
// Expose canvas's Path2D / CanvasGradient as globals (chart.js + plugins expect them).
...
async function renderChartJs(
  ...
  if (typeof untrustedChart === 'string') {
    // Try to parse as strict JSON first — if it succeeds, treat as a safe
    // object so we never reach new Function(). This is the common case for
    // internal callers that always send JSON chart configs (not JS functions).
    try {
      const parsed = JSON.parse(untrustedChart);
      if (parsed && typeof parsed === 'object') {
        untrustedChart = parsed;
      }
    } catch (_) {
  ...
  if (typeof untrustedChart === 'string') {
    if (process.env.DISABLE_JS_CHARTS) {
      // Strict mode for untrusted / public self-host: refuse JS function configs
      // instead of evaluating them. Only strict-JSON chart configs are allowed.
      return Promise.reject(
        new Error('JS chart configs are disabled (DISABLE_JS_CHARTS). Send a JSON chart config.'),
      );
    }
    // The chart may contain JavaScript, evaluated below via new Function.
    // WARNING: new Function is NOT a sandbox — the body runs with access to Node
    // globals (e.g. `process`), so a hostile config can achieve RCE. Set
    // DISABLE_JS_CHARTS=1 when this endpoint is exposed to untrusted input.
    try {
